...
import EditBalance from "./fragement/EditBalance";
const BalanceSetting = () => {
    ...
    const handleAfterEdit = (idx, data, err, errs) => {        
        idx == -1 ? apis.balancePost(data).then(ret => {
            apiResult(ret, (id) => {
                let cc = [...datas]
                cc.unshift({
                    id: id,
    ...
    const handleEdit = (idx, item) => {
        ...
        setRightDrawer(<EditBalance data={item} onSubmit={(data, err, errs) => { handleAfterEdit(idx, data, err, errs) }} onClose={() => { setRightDrawer(false) }} />)
    }
    const handleAdd = () => {
        setAutoCloseRightDraw(false)
        setRightDrawer(<EditBalance data={{ id: 0 }} onSubmit={(data, err, errs) => { handleAfterEdit(-1, data, err, errs) }} onClose={() => { setRightDrawer(false) }} />)
    }
    const handleDelete = (idx, item) => {
        apis.balanceDelete(item.id).then(ret => {
            apiResult(ret, data => {
                let cc = [...datas]
                cc[idx].status = 0
                setDatas(cc)
    ...
    const handleEnable = (idx, item) => {
        apis.balancePut({ id: item.id, status: 1 }).then(ret => {
            apiResult(ret, data => {
                let cc = [...datas]
                cc[idx].status = 1
                setDatas(cc)
    ...
    const TableHeader = [
        { name: 'id', showName: 'ID' },
        { name: 'type', showName: 'Type'},
        { name: 'level', showName: 'Privillege' },
        {

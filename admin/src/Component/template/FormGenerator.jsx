            ...
            return <Paper key={index} sx={{ padding: 2, marginTop: 2 }}>
                ...
                {item.items.map((subitem, index1) => {
                    if (subitem.type === 'text') {
                        return <Typography key={index1} component="div" variant="body1" >
                            <div style={{ width: "100%", }} dangerouslySetInnerHTML={{ __html: subitem.label }}></div>
                        </Typography>
                        ...
                        return <LocalizationProvider key={index1} dateAdapter={AdapterDateFns} margin="normal" >
                            <DatePicker
                                ...
                                onChange={(newdate) => { onChange && onChange(index, index1, formatDate(newdate)) }}

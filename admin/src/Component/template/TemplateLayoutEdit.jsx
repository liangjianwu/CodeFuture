import { Box, MenuList, ListItemIcon, List, ListItem, Divider, MenuItem, ListItemText, Grid, Paper, Alert, IconButton, Toolbar, Typography } from "@mui/material"
import { useState } from "react"
import { Close, Delete, Image, Link, RemoveRedEye, Save, SmartButton, Splitscreen, Subtitles, ViewHeadline,Title } from "@mui/icons-material";

import { EditForm, EditImage, EditUrl, EditButton, EditPanel, EditImages } from "./TemplateEditForm";
import { TplButton, TplImage, TplSubtitle, TplText, TplTitle, TplUrl, TplImages } from "./TemplateComponent";
...
const TemplateLayoutEdit = (props) => {
    ...
    const handleObjClicked = (index, index1, index2, obj) => {
        if (['title', 'subtitle', 'text'].indexOf(obj.type) >= 0) {
            ...
            setRightPanel(<EditImages obj={obj} onEdit={(value) => { handleComponentEdit(index, index1, index2, value) }} />)
        }
    }
    const handlePreview = () => {
    ...
    return <Box sx={props.sx}>
        <Grid container spacing={2}>
            <Grid item xs={2}>
                <Paper>
                    <MenuList>
                        ...
                        <MenuItem onClick={() => { handleAdd('images') }} disabled={selected.length > 0}>
                            <ListItemIcon>
                                <Image fontSize="small" />
                            </ListItemIcon>
            ...
            <Grid item xs={5} sx={{ maxHeight: "700px", overflow: "auto" }}>
                    ...
                    <Grid container spacing={1} onClick={(e) => { e.preventDefault(); e && e.stopPropagation && e.stopPropagation(); handleSelected() }}
                        ...
                        {template.map((item, index) => {
                            ...
                            return item.map((child, index1) => {
                                ...
                                return <Grid key={index1} sx={{ background: panel && panel.backgroundColor ? panel.backgroundColor : '#fff', minHeight: 30, padding: 1, paddingLeft: panel && panel?.paddingLeft, paddingRight: panel && panel?.paddingRight, border: "1px " + (isSelected1 ? "solid " : "dashed ") + "grey" }}
                                    ...
                                    {child.map((subchild, index2) => {
                                        ...
                                        if (subchild.type !== 'panel') {
                                            return <Box key={index2} sx={{ border: "1px " + (isSelected2 ? "solid grey" : "dashed grey"), padding: 1, }} onClick={(e) => { e.preventDefault(); e && e.stopPropagation && e.stopPropagation(); handleSelected(index, index1, index2); handleObjClicked(index, index1, index2, subchild) }}>
                                                ...
                                                {['images'].indexOf(subchild.type) >= 0 && <TplImages data={subchild} />}
                ...
                {preview && <Paper style={{ padding: 5 }} >
                    ...
                    <Box sx={{ padding: 0, borderColor: "#ccc" }}>
                        <div style={{ width: "100%", }} dangerouslySetInnerHTML={{ __html: preview }}></div>
                    </Box>
                </Paper>}
            </Grid>
            <Grid item xs={3}>
            ...
            <Grid item xs={2}>
                {rightPanel && props.allowVariables && <Paper sx={{ padding: 1, maxHeight: 600, overflow: "auto" }} >
                    <Typography component={"h4"}>Varibales</Typography>                    
                    <List dense={true}>
                        <ListItem>
                            <ListItemText sx={{ marginTop: 0, marginBottom: 0 }}
                                primary={"%_customer_name%"}
                                secondary={"Customer name"}
                            />
                        </ListItem>
                        <Divider />
                        {props.variables&&props.variables.map((v, i) => {

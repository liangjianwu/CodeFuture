    ...
    return (<>
        ...
        {event && <Container component="main" maxWidth="sm">
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 4 }}>
                ...
                <Box sx={{ width: '100%' }}>
                    {step < 2 && event.apply == 1 && <FormGenerator form={form} onChange={handleChange} />}                    
                    {apply && step === 2 && event.apply == 1 && <Paper sx={{ p: 2, mt: 2, borderColor: '#059c' }}>
                        <Typography variant='body1' color="#ff5722" sx={{ mt: 2, mb: 2 }}>You have submitted successfully</Typography>
                        <Typography variant='body1'>Application number:  <span style={{ fontWeight: 'bold', color: '#ff5722' }}>{apply && apply.id}</span></Typography>
                    </Paper>}
                    {error && <Alert severity='error' onClose={() => setError()} sx={{ mt: 1, mb: 1 }}>{error}</Alert>}
                    {step === 1 && event.apply == 1 && <Button sx={{ marginTop: 2 }} fullWidth variant='contained' onClick={() => handleSubmit(true)}>{apply ? 'Modify' : 'Submit'}</Button>}

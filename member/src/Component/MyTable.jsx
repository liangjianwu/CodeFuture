...
export default function MyTable(props) {
    const { totalRow, rows, headers, OpentionComponent, checkbox, onChangePage, onChangeRowsPerPage, onSelected } = props
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(props.rowsPerPage);
    const [selected, setSelected] = useState([])
    // Avoid a layout jump when reaching the last page with empty rows.
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - totalRow) : 0;
    ...
    return (<Paper sx={{ width: '100%', overflow: 'hidden' }}>
        <TableContainer component={Paper} >
            <Table sx={{ minWidth: 500 }} size={rows.length > 10 ? 'small' : ''} aria-label="custom pagination table">
                <TableHead>
                    <TableRow sx={{ background: '#00f1' }}>
                        ...
                        {headers.map((item, index) => {
                            return <TableCell key={index} align={index > 0 ? 'left' : 'left'}>{item.showName}</TableCell>
                        })}
                ...
                <TableBody>
                    {rows.map((row, index) => {
                        ...
                        >
                            ...
                            /></TableCell>}
                            {headers.map((hd, i) => {
                                return i == 0 ? <TableCell id={labelId} key={i} onClick={(event) => handleItemClick(event, row.id)} component="th" scope="row">{row[hd.name]}</TableCell> :
                                    <TableCell onClick={(event) => handleItemClick(event, row.id)} key={i} align="left">{row[hd.name]}</TableCell>
                            })}

...
const SignUp = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if(!acceptTerms) {
      setError('Please accept the agreement to sign up!')
      return
    }
    ...
    postdata.firstname = postdata.firstname.trim()
    postdata.lastname = postdata.lastname.trim()
    
    apis.signUp(postdata).then(ret => {
      apiResult(ret, (data) => {
        setUserSession(data)
    ...
    <Box sx={{ marginTop: 8, display: 'flex', flexDirection: 'column', alignItems: 'center', }}>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ mt: 3 }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            <PasswordTextField sx={{width:'100%'}} required fullWidth name="passwd" label="Password" type="password" id="passwd" 
          ...
          <Grid item xs={12}>
            <FormControlLabel
              ...
              label={<Typography variant='body2'>I've read and agree to <a href="/user/terms" target="_blank">CodeFuture User Agreement</a>.</Typography>} />
          </Grid>
        </Grid>
        ...
        <Grid container justifyContent="flex-end">
          <Grid item>
            <Link href="/user/signin" variant="body2"> Already have an account? Sign in </Link>
          </Grid>
        </Grid>
      </Box>
    </Box>
